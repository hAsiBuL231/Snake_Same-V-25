                ...
                child: Column(children: [
                  Row(mainAxisAlignment: MainAxisAlignment.start, children: [
                    CachedNetworkImage(
                      ...
                      imageBuilder: (context, imageProvider) => Container(
                          decoration: ShapeDecoration(shape: CircleBorder(side: BorderSide(color: Colors.blue, width: 5))),
                          child: CircleAvatar(radius: 50, backgroundImage: CachedNetworkImageProvider(imageUrl))),
                      placeholder: (context, url) => CircularProgressIndicator(),
                      errorWidget: (context, url, error) => CircleAvatar(child: Icon(Icons.account_circle, size: 50), radius: 50, backgroundColor: Colors.blue),
                    ),
                    /*CachedNetworkImage(
                        imageUrl: imageUrl,
                  ...
                  Row(children: [
                    const Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      SizedBox(height: 4.0),
                      Text('1.2k', style: TextStyle(fontSize: 24, color: Colors.white, fontWeight: FontWeight.bold))
                    ]),
                    const SizedBox(width: 16.0),
                    const Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text('Following', style: TextStyle(fontSize: 16, color: Colors.white)),
                      SizedBox(height: 4.0),
                      Text('980', style: TextStyle(fontSize: 24, color: Colors.white, fontWeight: FontWeight.bold))
                    ]),
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  ListTile(leading: const Icon(Icons.email), title: const Text('Email'), subtitle: Text('$userEmail')),
                  ListTile(leading: const Icon(Icons.phone), title: const Text('Phone'), subtitle: Text(phone)),
                  ListTile(leading: const Icon(Icons.location_on), title: const Text('Location'), subtitle: Text(location)),
                  ListTile(leading: const Icon(Icons.calendar_today), title: const Text('Date of Birth'), subtitle: Text(dob)),
                  ListTile(leading: const Icon(Icons.language), title: const Text('Languages'), subtitle: Text(language)),
                ],
              ),
            ),
            Padding(
              padding: const EdgeInsets.all(16.0),
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                const Text('App Usage', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                const SizedBox(height: 8.0),
                StreamBuilder(
                    stream:
                        FirebaseFirestore.instance.collection('user_list').doc(userEmail).collection('scores').orderBy("score", descending: true).limit(1).snapshots(),
                    builder: (context, snapshot) {
                      if (snapshot.hasData) {
                        if (snapshot.data!.docs.length < 1) {
                          return ListTile(leading: Icon(Icons.leaderboard_outlined), title: Text('Highest Score'), subtitle: Text('No data available'));
                        }
                        return ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              return ListTile(leading: Icon(Icons.leaderboard_outlined), title: Text('Highest Score'), subtitle: Text('Score: $score'));
                            });
                      }
                      return ListTile(leading: Icon(Icons.leaderboard_outlined), title: Text('Highest Score'), subtitle: Text('Something went wrong'));
                    }),
                ListTile(leading: const Icon(Icons.calendar_today), title: const Text('Joined'), subtitle: Text(joined)),
                ListTile(leading: const Icon(Icons.timer), title: const Text('Total App Usage'), subtitle: Text('Days: $difference')),
                ListTile(leading: Icon(Icons.star), title: Text('Favorite Feature'), subtitle: Text('Push Notifications')),
              ]),
            ),
            Center(
                child: FilledButton(
